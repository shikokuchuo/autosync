...
R sync server and client for [Automerge](https://automerge.org/) CRDT
documents. Implements the `automerge-repo` WebSocket protocol, enabling
R to serve as a synchronization hub for Automerge clients in R,
JavaScript, Rust, and other languages, and to act as a client that
...
The `share` parameter controls which documents are announced to clients
and which requests are allowed. It accepts `TRUE`, `FALSE`, `NA`, or a
function returning one of these per client and document:
...
Combine with authentication and use a `share` function for fine-grained
access control. The function receives the `client_id` and `doc_id`, and
